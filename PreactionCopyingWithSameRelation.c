		...
		if (ITEM_list_all_revs(tSourceItem, &iCount, &tRevList) == iStatus)//Retrieves all revisions based on item tag.
		{
			if (tRevList != NULL)
			{
				for (i = 0; i < iCount; i++)
				{
					...
					{
						if(GRM_list_secondary_objects(tSourceRev,NULLTAG,&iSecCount,&tSecList));
						//count List of tag quadruplet
						...
						if (tSecList != NULL)
						{
							...
							{
								POM_class_of_instance(tSecList[j].secondary, &tClassId);//
								if (tClassId != NULLTAG)
								{
									...
									if (cClassName != NULL)
									{
										...
										if (k == 0)
										{
											GRM_create_relation(tRevList[i - 1], tSecList[j].secondary, tSecList[j].relation_type, NULLTAG, &tNewRelation);//Creates a relation of the specified type.
