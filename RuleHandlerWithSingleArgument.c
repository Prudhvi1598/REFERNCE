...
extern DLLAPI int handler_execute_callbacks1(int *decision, va_list argv)
{
	*decision = ALL_CUSTOMIZATIONS;
	...
	EPM_register_rule_handler("Rule_Handler_with_Single_Argument", "Rule_Handler_with_Single_Argument", (EPM_rule_handler_t)check_rule);
						...
						if (tClassId != NULLTAG)
						{
							...
							if (cClassName != NULL)
							{
								...
								if (tc_strcmp(cClassName, "ItemRevision") == ITK_ok)
								{
									...
									check(GRM_list_secondary_objects_only(tAttachments[i], NULLTAG, &iSecCount, &tSecObjects));//lists all secondary objects
									if (iSecCount != 0 && tSecObjects != NULL)
									{
										for (int k = 0; k < iSecCount; k++)
										{
											check(WSOM_ask_object_type2(tSecObjects[k], &cObjectType));// Retrieves the object type of the specified WorkspaceObject.
											
											if (cObjectType != NULL)
											{
										
												if (tc_strcmp(cObjectType, cStatus) == 0)
												{
													return EPM_go;
												}
									...
									{
										check(WSOM_ask_object_id_string(tAttachments[i], &cObjectName));//Retrieves the object ID of the specified WorkspaceObject.
										if (cObjectName != NULL)
										{
