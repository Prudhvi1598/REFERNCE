//------ITEM DELETE TASK where it is referenced------------------
...
int deletingFromBom(tag_t tBOMviewRevisionTag,tag_t ItemTag)
{
	...
	tag_t tBOmViewItem = NULLTAG;
	check(BOM_create_window(&tWindow));
	if (tWindow != NULLTAG)
	{
		printf("BOM window created\n");
		check(PS_ask_bom_view_of_bvr(tBOMviewRevisionTag, &tBomView));
		check(PS_ask_item_of_bom_view(tBomView, &tBOmViewItem));
		check(BOM_set_window_top_line(tWindow, tBOmViewItem, NULLTAG, tBomView, &tTopline));
		if (tTopline != NULLTAG)
		{
			printf("Top line set\n");
			...
			{
				check(PS_delete_bvr(tBOMviewRevisionTag));
			}
		}
	}
