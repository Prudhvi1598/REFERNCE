...
int ITK_user_main(int argc, char* argv[])
{
	...
	if (iFail == ITK_ok)
	{
		...
		iFail = ITEM_find_item("000336", &titem);
		iFail = ITEM_find_rev("000336", "A", &trev);
		iFail = PS_create_bom_view(tViewType, "swamy", "", titem, &tBOMview);
		if (iFail == ITK_ok)
		{
			AOM_save_without_extensions(tBOMview);
			printf("BOMview Created Successfully!\n\n");
			iFail = PS_create_bvr(tBOMview, "kumar", "", false, trev, &tbvr);
			if (iFail == ITK_ok)
			{
				...
				if (iFail == ITK_ok)
				{
					...
					iFail = BOM_line_add(ttopline, NULLTAG, trev, tbvr, &tnewline);
					if (iFail == ITK_ok)
					{
