...
extern DLLAPI int PLM_execute_callbacks1(int *decision, va_list argv)
{
	*decision = ALL_CUSTOMIZATIONS;
	...
	iStatus = METHOD_find_method("BOMLine", "BOMLine_add", &tMethodId);
...
extern DLLAPI int PLM_add_precondition(METHOD_message_t *msg, va_list argv)
{
	...
	if (tParent != NULLTAG)
	{
		check(AOM_ask_value_string(tParentTag, "bl_item_object_type", &cPvalue));
		if (cPvalue != NULL)
		{
			if (tItem != NULLTAG && tItemRev == NULLTAG)
			{
				check(AOM_ask_value_string(tItem, "object_type", &cCvalue));
			}
			else if (tItem == NULLTAG && tItemRev != NULLTAG)
			{
				check(ITEM_ask_item_of_rev(tItemRev, &tItemTag));
				if (tItemTag != NULLTAG)
				{
					check(AOM_ask_value_string(tItemTag, "object_type", &cCvalue));
				}
	...
	if (tc_strcmp(cPvalue, cCvalue) == 0)
	{
